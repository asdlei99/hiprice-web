        ...
        <v-card flat>
          ...
          <v-layout v-if="!product.isPriceRange" row wrap>
            ...
            <v-flex xs5 offset-xs6 v-if="remind.decreaseOption !== null && remind.decreaseOption.value === 1">
              <v-text-field v-model="remind.decreaseValue" prefix="￥" label="价格低于" type="number" :rules="[remind.priceValidate]"></v-text-field>
            </v-flex>
            <v-flex xs5 offset-xs6 v-if="remind.decreaseOption !== null && remind.decreaseOption.value === 2">
              <v-text-field v-model="remind.decreaseValue" suffix="%" label="降幅超过" type="number" :rules="[remind.rateValidate]"></v-text-field>
            </v-flex>
          </v-layout>
          ...
          <v-layout v-if="!product.isPriceRange" row wrap>
            ...
            <v-flex xs5 offset-xs6 v-if="remind.increaseOption !== null && remind.increaseOption.value === 1">
              <v-text-field v-model="remind.increaseValue" prefix="￥" label="价格高于" type="number" :rules="[remind.priceValidate]"></v-text-field>
            </v-flex>
            <v-flex xs5 offset-xs6 v-if="remind.increaseOption !== null && remind.increaseOption.value === 2">
              <v-text-field v-model="remind.increaseValue" suffix="%" label="涨幅超过" type="number" :rules="[remind.rateValidate]"></v-text-field>
            </v-flex>
          </v-layout>
    ...
    methods: {
      getRemind (productID) {
        this.$axios.get('remind?u=' + global.Store.user + '&p=' + productID).then(resp => {
          ...
          this.remind.decreaseValue = r.remind_decrease_value
          ...
          this.remind.increaseValue = r.remind_increase_value
        }).catch(() => {
          console.log('request failed: getRemind for ' + productID)
      ...
      save () {
        ...
        if (dov !== 0) {
          if (this.remind.decreaseValue <= 0) {
            return
          }
          if (dov === 1 && this.remind.decreaseValue > 999999) {
            return
          }
          if (dov === 2 && this.remind.decreaseValue > 99) {
            return
          }
        }
        if (iov !== 0) {
          if (this.remind.increaseValue <= 0) {
            return
          }
          if (iov === 1 && this.remind.increaseValue > 999999) {
            return
          }
          if (iov === 2 && this.remind.increaseValue > 99) {
            return
          }
        }
        let data = JSON.stringify({
          ...
          remind_decrease_value: parseFloat(this.remind.decreaseValue),
          remind_increase_option: this.remind.increaseOption === null ? 0 : this.remind.increaseOption.value,
          remind_increase_value: parseFloat(this.remind.increaseValue)
        })
